...
function getDayColorClass(key) {
  const count = reservations.filter((entry) => entry.date === key && (entry.status === "reserved" || entry.status === "occupied")).length;
...
function renderStats() {
  ...
  freeCountEl.textContent = 100 - reserved - occupied;
...
function renderDetailPanel() {
  detailLog.innerHTML = "";
  selectedUmbrellaLabel.textContent = selectedUmbrella ? `Umbrella ${selectedUmbrella}` : "Select an umbrella";
  ...
  if (entry.createdAt) {
    history.push(`Created by ${resolveName(entry.createdBy)} at ${new Date(entry.createdAt).toLocaleString()}`);
  }
  if (entry.occupiedAt) {
    history.push(`Occupied by ${resolveName(entry.occupiedBy)} at ${new Date(entry.occupiedAt).toLocaleString()}`);
  }
...
async function handleReserve(event) {
  ...
  const payload = {
    ...
    createdAt: reusingFreeRecord ? new Date().toISOString() : existing?.createdAt || new Date().toISOString(),
    date: dateKey,
  };
  try {
...
async function handleOccupy(arrived = false) {
  ...
  if (!guestDetails) return alert("Guest name is required to mark an umbrella as occupied.");
  const dateKey = formatDateKey(selectedDate);
  const payload = {
    umbrellaNumber: selectedUmbrella,
    status: "occupied",
...
function attachAuthListener() {
  if (!auth) {
    showLoginMessage("Authentication is not configured.");
    return;
  }
